...
function isValidated(textInput, dropdownValue) {
  ...
  if (textInput === "") {
    ...
    document.getElementById("itemInput").classList.remove("border-red-500");
  }

  if (dropdownValue === "") {
    document.getElementById("categorySelect").classList.add("border-red-500");
  } else {
    document.getElementById("categorySelect").classList.remove("border-red-500");
  }
...
document.addEventListener("DOMContentLoaded", function () {
  const addButton = document.querySelector('.bg-sky-600');
